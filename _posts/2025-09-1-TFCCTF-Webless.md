...
## Finding bugs
i assumed that we are dealing with xs-leak (idk why i thought that). But my assumption was cut short by the CSP *only* the `/post/id` had which is `Content-Security-Policy : script-src 'none'; style-src 'self'` alr... maybe i can import post A into post B and cloud the post A with css selectorss??.. Nope.. nothing..  
...
But the attack seems unclear.. how can we take advantage of a user(bot) that is already logged in.. can we make the user access the invalid page and again the post/0 page without manual login??  yess we can!  
 ...
 2. The XSS opens a new tab to post/0(flag) and the new tab does maintain the session.
